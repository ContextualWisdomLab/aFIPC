...
autoFIPC <-
  function(
    ...
    nItems <- NA_integer_
    if (is.data.frame(newformXData) || is.matrix(newformXData)) nItems <- ncol(newformXData) # ⚡ Bolt: removed as.data.frame() to avoid O(N) memory allocation
    else if (is.data.frame(oldformYData) || is.matrix(oldformYData)) nItems <- ncol(oldformYData) # ⚡ Bolt: removed as.data.frame() to avoid O(N) memory allocation
