...
## 2024-07-13 - R 언어에서 데이터 프레임 값 갱신 시 2차원 인덱싱 O(N) 병목 최적화
**Learning:** R에서 데이터 프레임의 특정 값을 변경할 때 df[idx, "col"] <- value 방식을 사용하면 내부적으로 [<-.data.frame 메서드가 호출되어 타입/차원 검사 및 전체 데이터 프레임의 메모리 복사 오버헤드가 발생합니다.
**Action:** 데이터 갱신 시 df$col[idx] <- value와 같이 직접 벡터 부분집합 할당(direct vector assignment)을 사용하면 C-레벨 벡터 갱신을 수행하고 메서드 디스패치 및 전체 메모리 복사를 방지하여 성능을 크게 개선할 수 있습니다.
