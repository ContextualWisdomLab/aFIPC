...
## 2024-08-25 - [R 벡터 최솟값 탐색 성능 최적화]
**Learning:** R에서 데이터 프레임이나 벡터의 최솟값/최댓값을 찾을 때 `sort(x)[1]` 또는 `names(sort(x))[1]`을 사용하면 O(N log N)의 불필요한 정렬 오버헤드가 발생함.
**Action:** O(N) 선형 시간 복잡도를 가지는 `which.min(x)` 또는 `which.max(x)`를 활용하여 `names(x)[which.min(x)]` 형태로 값을 추출해야 함.
