...
## 2024-09-08 - R 언어에서 고유값 개수 계산 시 `stats::na.omit()` 대신 `sum(!is.na())` 사용 최적화
**Learning:** `length(stats::na.omit(unique(x)))`는 `stats::na.omit`의 메서드 디스패치 및 `na.action` 속성 할당으로 인해 불필요한 오버헤드가 발생합니다. 반복문 내부에서 사용될 경우 이 오버헤드는 성능 저하의 원인이 됩니다.
**Action:** 논리형 인덱스의 합을 구하는 방식인 `sum(!is.na(unique(x)))`을 사용하여 빠르고 효율적으로 고유한 non-NA 값의 개수를 계산함으로써 성능을 최적화합니다.
