...
## 2026-09-09 - R 언어에서 행렬을 데이터프레임으로 불필요하게 형변환하는 병목 최적화
**Learning:** R에서 단순히 열 개수를 구하기 위해 `ncol(as.data.frame(matrix))`와 같이 `as.data.frame()`을 호출하면 불필요한 O(N) 메모리 할당과 복사 오버헤드가 발생합니다. `ncol()`은 행렬(matrix) 객체를 기본적으로 지원하므로 강제 형변환이 필요하지 않습니다.
**Action:** 데이터 구조의 열 개수 등을 확인할 때 불필요한 `as.data.frame()` 강제 형변환을 제거하고 원래 객체에 대해 `ncol()`을 직접 호출하여 성능 저하와 메모리 오버헤드를 방지합니다.
