...
## 2024-07-13 - R 언어에서 stats::na.omit 오버헤드를 논리 연산 합산으로 최적화
**Learning:** R에서 고유값 중 NA를 제외한 개수를 셀 때 `length(stats::na.omit(unique(x)))`를 사용하면, `stats::na.omit` 내부의 메소드 디스패치(method dispatch)와 `na.action` 속성 할당(attribute allocation)으로 인해 불필요한 연산 및 메모리 오버헤드가 발생합니다.
**Action:** NA가 아닌 값의 개수만 필요한 경우, `sum(!is.na(unique(x)))`와 같은 논리형 배열의 합(vectorized logical summing)으로 대체하여 오버헤드를 없애고 연산 속도를 대폭(O(N) 상수 인자 개선) 향상시킵니다.
