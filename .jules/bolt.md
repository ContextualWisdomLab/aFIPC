...
## 2024-08-07 - O(N log N) Sorting Overhead for Minimum/Maximum
**Learning:** Finding the minimum or maximum element in an array using `names(sort(x))[1]` incurs unnecessary O(N log N) sorting overhead, which can become a bottleneck when called repeatedly in loops or for large arrays.
**Action:** Always prefer `names(x)[which.min(x)]` or `names(x)[which.max(x)]` for O(N) linear time complexity when only the extreme value is needed.
