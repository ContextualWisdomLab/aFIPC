...
## 2026-09-26 - Avoid as.data.frame() when checking ncol() on matrices in R
**Learning:** In R, calling `as.data.frame()` on a matrix solely to use `ncol()` triggers an expensive O(N) memory allocation and copy. Since `ncol()` natively supports matrices, the coercion is completely redundant and causes significant performance overhead for large datasets.
**Action:** Replace `ncol(as.data.frame(x))` with `ncol(x)` when `x` is known to be a matrix or a data.frame. This eliminates unnecessary data coercion overhead and reduces execution time.
