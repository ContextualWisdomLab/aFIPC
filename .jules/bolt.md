...
## 2025-09-26 - R 언어에서 고유한 non-NA 값 개수 계산 최적화
**Learning:** R에서 데이터의 고유값 중 NA가 아닌 값의 개수를 셀 때 `length(stats::na.omit(unique(x)))`를 사용하면 `na.omit()` 함수의 호출 시 S3 메서드 디스패치와 추가적인 속성(attribute) 할당 오버헤드가 발생하여 성능 저하를 일으킬 수 있습니다.
**Action:** `sum(!is.na(unique(x)))`와 같이 벡터화된 논리 연산을 사용하면 함수 호출 및 메모리 할당 오버헤드 없이 O(N) 탐색 내에서 빠르고 효율적으로 개수를 산출할 수 있습니다.
