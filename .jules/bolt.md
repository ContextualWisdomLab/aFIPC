...
## 2024-05-18 - [Optimize Finding Extremes in Vectors]
**Learning:** In R, avoid using `sort(x)[1]` or `names(sort(x))[1]` to find the minimum or maximum element, as it incurs O(N log N) overhead.
**Action:** Always prefer `which.min(x)` or `which.max(x)` (e.g., `names(x)[which.min(x)]`) for O(N) linear time complexity when finding extremes.
