...
## 2024-07-18 - R 언어에서 최소/최대값 탐색 시 불필요한 O(N log N) 정렬 최적화
**Learning:** R에서 벡터의 최소값이나 최대값을 가지는 요소의 이름이나 인덱스를 찾을 때 `names(sort(x))[1]`과 같이 전체를 정렬하는 방식을 사용하면 불필요한 $O(N \log N)$ 오버헤드가 발생합니다. 전체 요소를 정렬할 필요가 없는 단일 항목 탐색에는 최적화되지 않은 접근입니다.
**Action:** 단순히 최소/최대값의 위치나 이름만 필요할 때는 `which.min()` 또는 `which.max()` 함수를 활용하여 `names(x)[which.min(x)]` 형태로 작성하면 선형 시간인 $O(N)$으로 성능을 크게 향상시킬 수 있습니다.
