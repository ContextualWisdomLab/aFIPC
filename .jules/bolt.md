...
## 2025-02-12 - R 언어에서 데이터프레임의 특정 열 이름 추출 시 병목 최적화
**Learning:** R에서 열 이름을 알아내기 위해 데이터프레임을 서브셋팅(subsetting)하는 작업(`colnames(df[cols])`)은 데이터 전체를 복사하는 O(N) 오버헤드를 발생시켜 심각한 성능 저하를 일으킵니다.
**Action:** 이를 `intersect(cols, colnames(df))`와 같이 집합 연산으로 대체하여 실제 데이터 복사 없이 열 이름만 가볍게 추출하도록 하여 O(1)에 가까운 성능으로 최적화해야 합니다. 이때 `cols`를 첫 번째 인자로 전달하여 원래 의도했던 열의 순서를 보존해야 합니다.
