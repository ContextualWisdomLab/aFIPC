...
## 2024-07-29 - [Optimized p-value minimum search]

**Learning:** For R performance optimization, avoid using `sort(x)[1]` or `names(sort(x))[1]` to find the minimum or maximum element, as it incurs O(N log N) overhead.
**Action:** Instead, use `which.min(x)` or `which.max(x)` (e.g., `names(x)[which.min(x)]`) for O(N) linear time complexity.
