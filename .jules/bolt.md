...
## 2024-05-18 - Fast Dataframe Assignment in R
**Learning:** In base R, 2D dataframe subsetting assignments (e.g. `df[idx, "col"] <- val`) suffer from significant `[<-.data.frame` method dispatch and dimensionality checking overhead.
**Action:** Replace them with direct vector assignments (e.g. `df$col[idx] <- val`) to bypass the overhead for measurable performance gains in tight loops or large datasets.
