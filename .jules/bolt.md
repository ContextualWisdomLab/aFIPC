...
## 2026-09-23 - R 언어에서 고유값 개수(unique count) 산출 시 stats::na.omit 오버헤드 최적화
**Learning:** R에서 고유값 중 NA를 제외한 개수를 셀 때 `length(stats::na.omit(unique(x)))`를 사용하면, `stats::na.omit` 함수가 S3 메서드 디스패치를 수행하고 `na.action` 속성을 할당하는 과정에서 상당한 오버헤드가 발생합니다. 이는 루프 내부나 빈번하게 호출되는 로직에서 성능 저하의 원인이 됩니다.
**Action:** 논리 인덱스를 합산하는 `sum(!is.na(unique(x)))` 방식으로 대체하여, 불필요한 속성 할당과 메서드 디스패치를 피함으로써 성능을 최적화해야 합니다.
