...
## 2024-08-12 - 최솟값 탐색 최적화
**Learning:** R에서 최솟값/최댓값을 찾을 때 `sort(x)[1]` 또는 `names(sort(x))[1]`을 사용하면 O(N log N)의 정렬 오버헤드가 발생합니다.
**Action:** 불필요한 정렬을 피하기 위해 선형 시간 복잡도 O(N)을 갖는 `which.min(x)` 또는 `which.max(x)`를 사용해야 합니다. (예: `names(x)[which.min(x)]`)
