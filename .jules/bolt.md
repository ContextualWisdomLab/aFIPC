...
## 2024-05-24 - [Optimize unique count calculation]
**Learning:** `length(unique(stats::na.omit(x)))` or `length(stats::na.omit(unique(x)))` carries significant overhead due to method dispatch and `na.action` attribute allocations.
**Action:** Use `sum(!is.na(unique(x)))` instead of `length(stats::na.omit(unique(x)))` for a significantly faster approach to counting unique non-NA values without memory overhead.
