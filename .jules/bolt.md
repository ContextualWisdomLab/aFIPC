...
## 2024-07-20 - R 언어에서 factor 생성 및 데이터 프레임 병합 병목 최적화
**Learning:** R에서 데이터 프레임의 새로운 열을 할당할 때 `data.frame(df, col)`을 사용하면 전체 메모리 할당 및 복사 오버헤드로 인해 O(N) 시간이 소요됩니다. 또한 `as.factor(c(...))`는 내부적으로 불필요하게 팩터 레벨(level)을 추론하는 오버헤드가 발생하여 대용량 데이터에서 속도가 저하됩니다.
**Action:** 팩터는 `factor(..., levels = c(...))` 형태를 사용하여 레벨을 명시적으로 선언함으로써 자동 추론에 의한 오버헤드를 줄이고, 새로운 열의 할당은 `df$col <- col` 문법을 직접 사용하여 O(1)에 가깝게 동작하도록 복사 오버헤드를 제거해야 합니다.
