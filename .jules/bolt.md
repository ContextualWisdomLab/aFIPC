...
## 2024-08-25 - [R 데이터 프레임 열 이름 추출 O(N) 복사 오버헤드 최적화]
**Learning:** R에서 데이터 프레임의 특정 열 이름들만 추출할 때 `colnames(df[cols])`와 같이 서브셋팅을 사용하면 불필요한 O(N) 메모리 복사가 발생함.
**Action:** 메모리 복사 없이 효율적으로 열 이름을 추출하고 순서를 유지하기 위해 `intersect(cols, colnames(df))`를 사용해야 함.
