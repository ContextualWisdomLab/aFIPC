...
## 2024-07-17 - 최솟값 탐색 시 O(N) which.min()을 사용하여 O(N log N) sort() 오버헤드 회피
**Learning:** R에서 최솟값을 가지는 요소의 이름을 찾을 때 `names(sort(x))[1]`을 사용하면 O(N log N)의 불필요한 정렬 오버헤드가 발생합니다. 최솟값 탐색은 O(N)의 선형 탐색으로 충분합니다.
**Action:** 항상 `names(sort(x))[1]` 대신 선형 탐색 함수인 `names(which.min(x))` 또는 `names(x)[which.min(x)]`를 사용하여 성능을 최적화해야 합니다.
