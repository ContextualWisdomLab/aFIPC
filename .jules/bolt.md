...
## 2024-09-03 - Vectorized column name extraction
**Learning:** In R, subsetting a dataframe inside `colnames()` (e.g., `colnames(df[cols])`) incurs an unnecessary O(N) memory allocation and deep copy of the underlying data.
**Action:** When extracting the intersection of column names, extract them directly from the target vector or object (e.g., `cols` or `colnames(df)`).
