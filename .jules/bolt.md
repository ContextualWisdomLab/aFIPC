...
## 2024-05-18 - R 고속 유니크 카운트 (na.omit 대체)
**Learning:** R에서 결측치가 아닌 고유값의 개수를 셀 때 `length(stats::na.omit(unique(x)))` 또는 `length(stats::na.omit(unique(x)))`를 사용하는 것은 객체 메서드 디스패치와 `na.action` 어트리뷰트 할당 오버헤드 때문에 비효율적이다. `sum(!is.na(unique(x)))`를 사용하면 이러한 오버헤드 없이 훨씬 더 빠르게 계산할 수 있다.
**Action:** 결측치가 아닌 고유값의 개수가 필요할 때마다 `sum(!is.na(...))` 논리 인덱싱 합산 패턴을 사용하자.
