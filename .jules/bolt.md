...
## 2024-07-13 - R 언어에서 데이터프레임 부분집합 추출을 통한 값 할당 최적화
**Learning:** R에서 특정 조건에 맞는 데이터프레임의 행과 열을 업데이트할 때, 이차원 서브셋팅(예: `df[df$idx == 'val', 'col'] <- new_val`)을 사용하면 메소드 디스패치(`[<-.data.frame`)가 발생하여 불필요한 차원 확인 및 데이터 복사가 일어나 오버헤드가 발생합니다.
**Action:** 이차원 서브셋팅 대신 직접 벡터 서브셋팅(예: `df$col[df$idx == 'val'] <- new_val`)을 사용하여 O(1) 수준의 리스트 접근과 C 레벨의 벡터 수정으로 성능을 최적화해야 합니다.
