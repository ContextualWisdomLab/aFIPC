...
## 2024-09-14 - R 언어에서 na.omit()을 활용한 고유값 개수 탐색 성능 최적화
**Learning:** R에서 `length(stats::na.omit(unique(x)))`와 같이 `stats::na.omit()`을 사용할 경우, 내부적으로 S3 메서드 디스패치가 발생하며 불필요한 `na.action` 속성(attribute) 할당 메모리가 추가로 사용됩니다. 루프 안에서 이러한 연산이 누적되면 심각한 병목이 될 수 있습니다.
**Action:** 결측치가 아닌 고유값의 개수를 셀 때는 메모리 복사 및 속성 할당 오버헤드가 없는 논리 인덱스 합산 방식인 `sum(!is.na(unique(x)))`으로 대체하여 메서드 오버헤드를 없애고 O(N) 연산 성능을 확보해야 합니다.
