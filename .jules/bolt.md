...
## 2024-07-13 - R 언어에서 ncol()을 위한 불필요한 as.data.frame() 변환 최적화
**Learning:** 행렬(matrix)을 `ncol()`로 열 개수를 구할 때, `as.data.frame()`으로 데이터프레임으로 변환하는 것은 불필요한 O(N) 메모리 할당 및 복사 오버헤드를 발생시킵니다.
**Action:** `ncol()`은 행렬에 대해서도 기본적으로 지원되므로, 대상 객체에 직접 `ncol()`을 호출하여 성능을 향상시키고 불필요한 메모리 복사를 방지해야 합니다.
