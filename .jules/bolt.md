...
## 2024-09-04 - R 언어에서 데이터프레임 부분집합의 열 이름 추출 시 발생하는 O(N) 메모리 할당 병목 최적화
**Learning:** R에서 `colnames(df[cols])`와 같이 서브셋팅된 데이터 프레임의 열 이름을 추출하면 불필요하게 데이터의 복사본이 생성되어 O(N) 메모리 할당 및 복사 오버헤드가 발생합니다. 특히 큰 데이터 프레임에 대해 이 작업을 반복 수행하면 매우 느려집니다.
**Action:** 데이터 프레임을 서브셋팅하여 열 이름을 얻는 대신, 사전에 알고 있는 명시적인 열 이름 리스트(예: `cols`)를 그대로 사용하거나, 원본 객체에서 직접 추출하여(`colnames(model$data)`) 불필요한 O(N) 서브셋팅 메모리 할당을 방지하고 O(1) 수준으로 성능을 크게 개선해야 합니다.
