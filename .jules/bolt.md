...
## 2025-02-12 - R 언어에서 고유값 개수 계산 시 stats::na.omit 오버헤드 최소화
**Learning:** R에서 데이터 내 고유값의 개수를 셀 때 `length(stats::na.omit(unique(x)))`를 사용하면 `stats::na.omit`이 내부적으로 메서드 디스패치 및 `na.action` 속성을 할당하면서 불필요한 오버헤드를 유발합니다. 이는 반복문 안에서 수행될 경우 성능 저하의 원인이 될 수 있습니다.
**Action:** `sum(!is.na(unique(x)))`로 변경하여 논리 연산의 합으로 대체함으로써 속성 할당과 관련된 오버헤드를 피하고 성능을 높일 수 있습니다.
