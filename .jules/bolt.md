...
## 2024-07-28 - R 언어에서 불필요한 as.data.frame 캐스팅으로 인한 성능 저하 최적화
**Learning:** R에서 데이터의 컬럼 수(`ncol()`)를 구하기 위해 매트릭스나 데이터 프레임 타입인 변수를 굳이 `as.data.frame()`으로 다시 변환하는 과정은 불필요한 메모리 복사 오버헤드(O(N))를 유발하여 애플리케이션의 속도를 저하시킵니다. 특히 행(row)이나 열(column)이 많은 매트릭스일 경우 그 비용은 매우 커집니다.
**Action:** 매트릭스 또는 데이터 프레임 변수에 대해 단순히 행/열 개수를 확인할 때는 `as.data.frame()`과 같은 타입 변환을 생략하고 직접 `ncol()` 또는 `nrow()`를 호출하여 불필요한 복사 오버헤드 및 O(N) 비용을 피해야 합니다.
