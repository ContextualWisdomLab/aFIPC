...
## 2024-05-24 - R 언어에서 루프 내 데이터 프레임 탐색 및 할당 최적화
**Learning:** `aFIPC.R` 코드 분석 중 데이터프레임에서 반복적인 부분 집합 2D 할당(`df[idx, "col"] <- val`)이 존재함을 확인했습니다. 이 작업은 내부적으로 복잡한 검증 과정을 거치기 때문에 느립니다.
**Action:** 이를 벡터화된 부분 집합 할당(`df$col[idx] <- val`)으로 변경하여 2D 데이터프레임 오버헤드를 우회하고 O(1) 리스트 접근 및 C레벨 벡터 수정을 통해 성능을 향상시켰습니다.
