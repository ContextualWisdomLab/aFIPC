...
## 2025-02-12 - R 언어에서 ncol() 호출 시 불필요한 as.data.frame() 변환에 따른 O(N) 오버헤드 제거
**Learning:** R에서 데이터프레임의 열 개수를 구하는 `ncol()` 함수는 매개변수로 주어진 값이 데이터프레임인지 행렬인지 구분 없이 동작합니다. `ncol(as.data.frame(x))`와 같이 `as.data.frame()`으로 굳이 형변환을 한 뒤 `ncol()`을 호출하는 경우 불필요한 메모리 할당(O(N)) 및 복사 오버헤드가 발생하여 성능을 크게 저하시킬 수 있습니다.
**Action:** `ncol()`을 호출할 때 굳이 데이터프레임으로 변환할 필요가 없는 경우에는 곧바로 변수를 인자로 전달하여 성능 병목을 예방해야 합니다.
