...
## 2025-07-17 - R 언어에서 데이터프레임 부분집합 업데이트 시 병목 최적화
**Learning:** R에서 데이터 프레임의 특정 항목을 업데이트할 때 2차원 부분집합(예: `df[idx, 'col'] <- val`)을 사용하면 매번 임시 객체를 생성하고 행렬 매칭 오버헤드가 발생하여 O(N)의 성능 저하를 일으킵니다.
**Action:** 2차원 부분집합 대신 직접적인 벡터 인덱싱(예: `df$col[idx] <- val`)을 사용하여 복사 오버헤드를 줄이고 데이터 프레임 업데이트 성능을 향상시켜야 합니다.
