...
## 2025-02-13 - [성능 최적화: 고유값 개수 계산]
**Learning:** R에서 고유한 NA가 아닌 값의 개수를 셀 때 `length(stats::na.omit(unique(x)))`를 사용하면 메서드 디스패치 및 `na.action` 속성 할당으로 인해 오버헤드가 발생하여 성능이 저하됩니다. 대신 `sum(!is.na(unique(x)))`를 사용하면 논리 인덱스 합산을 통해 성능을 크게 향상시킬 수 있습니다.
**Action:** 데이터 프레임 열의 고유값 개수를 비교할 때 `sum(!is.na(unique(x)))`를 사용하여 오버헤드를 최소화하고 성능을 최적화합니다.
