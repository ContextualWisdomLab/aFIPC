...
## 2024-07-13 - R 언어에서 na.omit을 사용한 고유값 개수 계산 최적화

**Learning:** `length(unique(stats::na.omit(x)))` 또는 `length(stats::na.omit(unique(x)))`를 사용하여 결측치가 아닌 고유값의 개수를 셀 때, `stats::na.omit` 함수는 메서드 디스패치(method dispatch) 및 `na.action` 속성 할당으로 인해 불필요한 오버헤드를 발생시킵니다. 데이터가 크거나 반복문 안에서 자주 호출될 경우 이는 병목이 될 수 있습니다.

**Action:** 결측치를 제외한 고유값의 개수를 계산할 때는 논리 벡터의 합을 구하는 `sum(!is.na(unique(x)))` 방식을 사용하여 `stats::na.omit`으로 인한 오버헤드를 제거하고 성능을 최적화해야 합니다.
