...
## 2024-10-25 - R 언어에서 고유 요소 개수 산출 최적화
**Learning:** R에서 고유값을 계산한 뒤 NA를 제외한 길이를 구할 때, `length(stats::na.omit(unique(x)))`나 `length(unique(stats::na.omit(x)))`를 사용하면 `stats::na.omit`으로 인해 불필요한 메서드 디스패치 및 할당 오버헤드가 발생합니다.
**Action:** 논리 인덱스를 합산하는 `sum(!is.na(unique(x)))`을 사용하여 메모리 및 성능 오버헤드를 최적화해야 합니다.
