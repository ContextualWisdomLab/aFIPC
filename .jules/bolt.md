...
## 2024-08-01 - [R 최솟값 탐색 최적화]
**Learning:** R에서 배열의 최솟값 또는 최댓값을 찾을 때 `sort(x)[1]` 또는 `names(sort(x))[1]` 방식을 사용하면 불필요한 O(N log N) 정렬 비용이 발생합니다.
**Action:** O(N)의 선형 시간 복잡도만 요구하는 `which.min(x)` 또는 `names(x)[which.min(x)]`를 사용하여 성능 최적화를 적용해야 합니다.
