...
**Learning:** R에서 데이터프레임의 특정 로우/컬럼을 업데이트할 때 `df[index, "col"] <- value` 형식의 2D 서브셋팅을 사용하면, 복잡한 `[<-.data.frame` S3 메서드가 호출되어 차원 검사, 팩터 레벨 확인, 깊은 복사(deep copy) 등의 막대한 오버헤드가 발생합니다.
**Action:** 데이터프레임 업데이트 시 `df$col[index] <- value` 형식의 1D 벡터 할당을 사용하여, 데이터프레임 메서드 디스패치를 우회하고 리스트 접근 및 C 레벨 벡터 수정으로 직접 처리하도록 변경함으로써 성능을 약 2배 향상시켜야 합니다.
