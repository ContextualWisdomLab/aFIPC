...
## 2025-02-13 - R 언어에서 최소/최대값 탐색 시 O(N log N) 병목 최적화
**Learning:** R에서 데이터의 최소값이나 최대값을 찾기 위해 `sort(x)[1]` 또는 `names(sort(x))[1]`과 같이 `sort()` 함수를 사용하면 O(N log N)의 정렬 오버헤드가 발생하여 불필요한 성능 저하를 초래합니다.
**Action:** 단순히 최소값이나 최대값이 필요한 경우에는 정렬 대신 O(N) 선형 탐색을 수행하는 `which.min(x)` 또는 `which.max(x)`를 활용(`names(x)[which.min(x)]` 등)하여 성능을 최적화해야 합니다.
