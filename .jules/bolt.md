...
## 2024-09-09 - [R 성능 최적화: stats::na.omit 및 데이터프레임 subset 피하기]
**Learning:** R에서 고유값 중 NA가 아닌 값의 개수를 셀 때 `length(stats::na.omit(unique(x)))` 보다 `sum(!is.na(unique(x)))` 가 메서드 디스패치 및 속성 할당 오버헤드가 없어서 훨씬 더 빠르다. 또한 컬럼명을 얻기 위해 데이터 프레임을 서브셋팅하는 것 (`colnames(df[cols])`)은 O(N)의 메모리 복사를 유발하므로 `intersect(cols, colnames(df))`를 사용하는 것이 더 효율적이다.
**Action:** 앞으로 R 코드 작성 시 `stats::na.omit`의 불필요한 사용을 자제하고 벡터화된 논리 연산을 사용하며, 메모리 복사를 유발하는 서브셋팅 연산을 주의하여 최적화할 것.
