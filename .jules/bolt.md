...
## 2025-02-12 - R 언어에서 매트릭스를 데이터 프레임으로 변환하여 ncol()을 호출하는 불필요한 O(N) 오버헤드 최적화
**Learning:** R에서 열 개수를 구하기 위해 `ncol()`을 호출할 때 `as.data.frame(matrix)`와 같이 강제 형변환(coercion)을 수행하면 데이터 크기에 비례하는 불필요한 O(N) 복사 오버헤드가 발생합니다. `ncol()` 함수는 매트릭스(matrix)와 데이터프레임(data.frame) 모두에 대해 기본적으로 동작하므로 형변환이 필요하지 않습니다.
**Action:** `ncol(as.data.frame(x))`를 `ncol(x)`로 직접 호출하여 메모리 할당과 복사 오버헤드를 방지하여 O(1) 연산으로 최적화합니다.
