...
## 2024-05-18 - R 언어에서 고유값 개수 계산 시 O(N) 병목 최적화
**Learning:** R에서 결측치가 아닌 고유값의 개수를 계산할 때 `length(stats::na.omit(unique(x)))` 또는 `length(stats::na.omit(unique(x)))`를 사용하면 `stats::na.omit`의 메서드 디스패치 및 `na.action` 속성 할당 오버헤드로 인해 심각한 성능 저하가 발생합니다.
**Action:** 논리 인덱스 합산 방식인 `sum(!is.na(unique(x)))`로 대체하여 함수 호출 비용을 줄이고 실행 속도를 크게 향상시켜야 합니다.
