...
## 2025-02-12 - R 언어에서 데이터프레임의 행렬 형변환 최적화
**Learning:** 행렬 또는 데이터프레임의 컬럼 개수를 구할 때 `as.data.frame()`으로 형변환하여 `ncol()`을 호출하는 것은 O(N)의 메모리 할당 및 복사 오버헤드를 발생시킵니다.
**Action:** R의 `ncol()` 함수는 행렬(matrix)과 데이터프레임 모두에 대해 기본적으로 동작하므로, 불필요한 `as.data.frame()` 형변환을 제거하고 객체에 직접 `ncol()`을 호출하여 O(1) 수준으로 최적화해야 합니다.
